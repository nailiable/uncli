...
export interface RawConfigReturn {
  ...
  fileName: 'uncli.yml' | 'uncli.yaml' | 'uncli.json' | 'unknown'
  ...
  const config = {
    ...
    readRawConfig(cache: boolean = true): RawConfigReturn {
      const configLayer: Array<Omit<RawConfigReturn, 'config'>> = [
        { fileName: 'uncli.yml', format: 'yml', filePath: join(cwd(), 'uncli.yml') },
        { fileName: 'uncli.yaml', format: 'yaml', filePath: join(cwd(), 'uncli.yaml') },
        { fileName: 'uncli.json', format: 'json', filePath: join(cwd(), 'uncli.json') },
      ]
      ...
      for (const layer of configLayer) {
        ...
        if (existsSync(layer.filePath) && readFileSync(layer.filePath, 'utf-8').toString().length > 0) {
          const builder = { config: readFileSync(layer.filePath, 'utf-8').toString(), format: layer.format, fileName: layer.fileName, filePath: layer.filePath }
          loadConfig._cache = builder
          return builder
        }
      }

      loadConfig._cache = { config: '{ "integrations": {} }', format: 'json', fileName: 'unknown', filePath: 'unknown' }
      return loadConfig._cache
    },
    /**
    ...
    load(cache: boolean = true, options?: LoadOptions): unknown {
      ...
      if (rawConfig.format === 'json')
        ...
        return load(rawConfig.config as string, options)
    },
    /**
     ...
     */
    loadRawConfig(cache: boolean = true, options?: LoadOptions): RawConfigReturn {
      const rawConfig = config.readRawConfig(cache)
      if (rawConfig.format === 'json')
        rawConfig.config = JSON.parse(rawConfig.config as string)
      else
        rawConfig.config = load(rawConfig.config as string, options)
      return rawConfig
    },
  }
  return config
